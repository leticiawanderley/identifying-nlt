...
def get_structural_errors():
    ...
    error_types = pd.read_csv('./data/error_codes/error_type_meaning.csv')
...
def process_scatter_plot_data(dataset_file, results_column):
    df = pd.read_csv(dataset_file)
    df[results_column] = np.where(df[results_column], 1, 0)
    return df
